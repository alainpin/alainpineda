...
four informality definitions, each broken down by sex, age group, education
level, and state. None of these cuts uses a custom definition: every one is
...
definiciones = ({
  ...
  TOSI2: {
    ...
    texto: "The same informal-sector count, taken as a share of the non-agricultural employed rather than of all employed. The informal sector is already non-agricultural, so this rate runs above the previous one."
  }
})
...
the original series INEGI publishes in its Banco de Información Económica
(BIE) before they reach this page. The shaded band
