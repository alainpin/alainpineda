...
Participación laboral desglosada por sexo, grupo de edad, nivel educativo y
entidad. Ningún corte
...
La participación difiere por sexo dentro de cada grupo de edad, aunque la
edad la dispersa más que el sexo. La brecha se muestra como (hombres −
mujeres) / hombres, así que no tiene unidades y los grupos de edad caben en
una sola gráfica. La línea en tinta es la brecha nacional sin abrir por
edad.
...
// Error estandar de la brecha g = (H - M) / H por el metodo delta:
// dg/dH = M / H^2, dg/dM = -1 / H, asi Var(g) ~ (M/H^2)^2 Var(H) + (1/H)^2 Var(M).
// Hombres y mujeres son dominios disjuntos de la misma encuesta; se tratan
// como independientes, que ignora la covarianza por estratos y UPM
// compartidos: la misma aproximacion que hace el resto del sitio al combinar
// dos estimaciones. Bandas y tooltips son +-1.96 de esto.
...
graficaBrechaEdad = () => {
  ...
  return Plot.plot({
    ...
    marks: [
      Plot.gridY({ stroke: "var(--rule)" }),
      // Banda de 95% solo en la linea nacional: seis bandas encimadas
      // taparian las lineas; cada serie lleva su intervalo en el tooltip.
      ...
      // La línea nacional también va en el tooltip. El texto de abajo promete
      // un intervalo para cada serie, y un pointer atado sólo a datosEdad
      // resuelve el hover sobre la línea de tinta al grupo de edad más cercano.
      Plot.tip([...datosEdad, ...total].filter(d => d.valor != null), Plot.pointer({ x: "fecha", y: "valor",
        title: d => `${d.anio}-T${d.trimestre} · ${d.grupo === "__total__" ? "Todas las edades" : d.grupo}\nbrecha ${d.valor.toFixed(3)} ± ${(1.96 * d.ee).toFixed(3)}\nhombres ${d.H}% · mujeres ${d.M}%` })),
      Plot.axisX({ stroke: "var(--rule)", tickSize: 0 }),
      Plot.axisY({ stroke: "var(--rule)", tickSize: 0 })
    ]
  });
...
El grupo más joven es la excepción. Entre las personas de 15 a 19 años la
...
muestral complejo de la ENOE. Para la brecha por edad, (hombres − mujeres) /
hombres, el intervalo sale del método delta aplicado a las dos estimaciones,
tratando a hombres y mujeres como dominios independientes; se dibuja como
banda en la línea nacional y aparece en el tooltip de todas las series. El corte por entidad usa las 32 estimaciones
